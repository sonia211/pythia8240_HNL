// HNLDecays.cc is a part of the PYTHIA event generator.
// Copyright (C) 2018 Philip Ilten, Torbjorn Sjostrand.
// PYTHIA is licenced under the GNU GPL v2 or later, see COPYING for details.
// Please respect the MCnet Guidelines, see GUIDELINES for details.

// Function definitions (not found in the header) for the HNLDecays class.
...
const int    HNLDecays::NTRYCHANNEL = 10;

// Number of times to try a decay sampling.
const int    HNLDecays::NTRYDECAY   = 10000;

// These numbers are hardwired empirical parameters,
// intended to speed up the M-generator.
const double HNLDecays::WTCORRECTION[11] = { 1., 1., 1.,
  2., 5., 15., 60., 250., 1250., 7000., 50000. };

//--------------------------------------------------------------------------

// Initialize the HNLDecays class with the necessary pointers to info,
...
void HNLDecays::init(Info* infoPtrIn, Settings* settingsPtrIn,
  ...
  HNLExt    = settingsPtr->mode("HNLDecays:externalMode");
  HNLMode   = settingsPtr->mode("HNLDecays:mode");
  HNLMother = settingsPtr->mode("HNLDecays:HNLMother");
  HNLPol    = settingsPtr->parm("HNLDecays:HNLPolarization");
...
// Main method of the HNLDecays class. Pass the index of the HNL requested
// to be decayed along with the event record in which the HNL exists. The
// HNL is then decayed with proper spin correlations as well any partner.
...
//  std::cout<<"[Inside HNLDecays 1]"<<std::endl;
  std::cout<<"HNLMode"<<HNLMode<<std::endl;
  ...
  // Rewrite it.. sister really important?
    ...
    for (int i = 0; i < int(sistersOut1.size()); ++i) {
      int sn = out1.id() == 9900015 ? -1 : 1;
      int id = event[sistersOut1[i]].id();
      if      (id == sn * 13 && tau == -1) tau = sistersOut1[i];
  ...
  correlated = false;
  //std::cout<<"abs(mediator.id())"<<abs(mediator.id())<<std::endl;
  //std::cout<<"internalMechanism(event): "<<internalMechanism(event)<<std::endl;
  
  if (idxOut1 != idxOut2 &&
      (abs(out2.id()) == 11 || abs(out2.id()) == 13 || abs(out2.id()) == 15)) {
    //std::cout<<"determine if correlated"<<std::endl;
    correlated = true;
  ...
  bool known = false;
  hardME = 0;
  if      (HNLMode == 4) known = internalMechanism(event);
  else if (HNLMode == 5) known = externalMechanism(event);
  else {
    if ((HNLMode == 2 && abs(mediator.id()) == HNLMother) || HNLMode == 3) {
      known       = true;
      correlated  = false;
      double sign = out1.id() == -9900015 ? -1 : 1;
      //std::cout<<"sign: "<< sign<<std::endl;      
      particles[2].rho[0][0] = (1 - sign * HNLPol) / 2;
      particles[2].rho[1][1] = (1 + sign * HNLPol) / 2;
      
      //std::cout<<"Inside HNLMode == 2 && abs(mediator.id()) == HNLMother) || HNLMode == 3"<<std::endl;
      //std::cout<<"particles[2].rho[0][0] = "<<particles[2].rho[0][0]<<std::endl;
      //std::cout<<"particles[2].rho[1][1] = "<<particles[2].rho[1][1]<<std::endl;
      //std::cout<<"HNL mode"<<HNLMode<<std::endl;
      //std::cout<<"HNL pol"<<HNLPol<<std::endl;
      
      
      //std::cout<<" in1.id() "<< in1.id()<<std::endl;
      //std::cout<<" in2.id() "<< in2.id()<<std::endl;
      
      //std::cout<<" out1.id() "<< out1.id()<<std::endl;
      //std::cout<<" out2.id() "<< out2.id()<<std::endl;
  ...
  if (!known) {
    //std::cout<<"Inside !known"<<std::endl;
    //std::cout<<"abs(mediator.id()):"<<abs(mediator.id())<<std::endl;
    //std::cout<<"correlated:"<<correlated<<std::endl;
    ...
    if (abs(mediator.id()) == 22)
      ...
      hardME = hmeGamma2TwoFermions.initChannel(particles);
      infoPtr->errorMsg("Warning in HNLDecays::decay: unknown correlated "
                        "tau production, assuming from unpolarized photon");
    } else {
      infoPtr->errorMsg("Warning in HNLDecays::decay: unknown uncorrelated "
  ...
  while (!accepted) {
    ...
    if (decayWeight > decayWeightMax)
      infoPtr->errorMsg("Warning in HNLDecays::decay: maximum "
  ...
  if (correlated) {
    ...
    writeDecay(event,children);
    //std::cout<<"inside correlated, after writeDecay:"<<correlated<<std::endl;
  }
...
bool HNLDecays::internalMechanism(Event&) {

  // Flag if process is known.
  bool known = true;
  //std::cout<<"mediator id"<<mediator.id()<<std::endl;
  ...
  // Produced from a D or B hadron decay with a single tau.
             ...
             && abs(out2.id()) == 13) {
    //std::cout<<"created from 521"<<std::endl;
...
bool HNLDecays::externalMechanism(Event &event) {

  // Flag if process is known.
  bool known = true;

  // Uncorrelated, take directly from SPINUP if valid.
  if (HNLExt == 0) correlated = false;
  if (!correlated) {
...
vector<HelicityParticle> HNLDecays::createChildren(HelicityParticle parent) {
  ...
  while (!decayed && decayTries < NTRYCHANNEL) {
    ...
    //std::cout<<"meMode"<<meMode<<std::endl;
  ...
  for(int i(0); i<children.size(); ++i){
    //std::cout<<"children["<<i<<"]:"<<children[i].id()<<std::endl;
  }
  
  //if (parent.idAbs() == 13 && children.size() == 4 && meMode == 22)
  if (parent.idAbs() == 9900015 && children.size() == 4 && meMode == 22)
  ...
  if (children.size() == 3) {
    if (meMode == 1521){
      ...
      //std::cout<<"hmeTau2Meson"<<std::endl;
    }
  ...
  // Three body decays.
    ...
    // Leptonic decay.
    if (meMode == 1531 || (parent.idAbs() == 9900015 && meMode == 22))
